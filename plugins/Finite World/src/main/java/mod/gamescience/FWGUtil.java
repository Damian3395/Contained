...
public class FWGUtil {	
	...
	public static float isWasteland(World w, float chunkX, float chunkZ) {
		...
		float distDiff = FWGUtil.euclidDist(spawnX, spawnZ, chunkX, chunkZ);
		
		if (distDiff > FiniteWorldGen.worldRadius) {
			return Math.min(1f, (distDiff-FiniteWorldGen.worldRadius)/5f);
		}
		return 0;
	}
