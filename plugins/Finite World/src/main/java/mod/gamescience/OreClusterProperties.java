...
/**
 * Represents generation properties for an ore "cluster" -- a series of
 * adjacent world chunks which contain a specific block in high density.
 */
public class OreClusterProperties {
	 ...
	 */
	public OreClusterProperties(Block type, int minVein, int maxVein, 
	...
	public void determineAllChunks(World w, int worldSizeRadius) {
		...
		if (spawnChunks.size() == 0) {
			int numClusters = FWGUtil.randomRange(this.clusterCount.min(), this.clusterCount.max());
			for(int i=0; i<numClusters; i++) {
				ArrayList<Point> bestFound = new ArrayList<Point>();
				for(int j=0; j<allowedRestarts; j++) {
					int clustSize = FWGUtil.randomRange(this.clusterSize.min(), this.clusterSize.max());
	...
	{		
		...
		for(int i=0; i<=3; i++) {
			...
			if (!used.contains(probe) && !spawnChunks.contains(probe)
						&& FWGUtil.isWasteland(w, probe.x, probe.y) == 0) 
			{
	...
	public static OreClusterProperties generateFromConfig(Configuration c,
		...
		return new OreClusterProperties(
			type,
