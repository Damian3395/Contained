...
/**
 * A client-sided GUI that visualizes data to the screen.
 */
public class VisualizerGUI extends Gui {
	...
	public VisualizerGUI(Minecraft mc) {
		super();
		this.mc = mc;
	...
	public void onRenderExperienceBar(RenderGameOverlayEvent event) {
		...
		if (guiRender) {
			...
			int y = 20;
			int diameter = 100;
			drawPieChart(x, y, diameter, occupationValues, DataVisUtil.occupationNames);
			...
			drawLineGraph(x, y+diameter+50, mc.displayWidth-x*2-20, (int)(diameter*1.5), chartDataPoints, 3,
					DataVisUtil.convertBaseline(chartData.get(DataVisUtil.MINING).toArray(new Double[chartData.get(DataVisUtil.MINING).size()])),
					DataVisUtil.convertBaseline(chartData.get(DataVisUtil.COOKING).toArray(new Double[chartData.get(DataVisUtil.COOKING).size()])),
					DataVisUtil.convertBaseline(chartData.get(DataVisUtil.FARMING).toArray(new Double[chartData.get(DataVisUtil.FARMING).size()])),
					DataVisUtil.convertBaseline(chartData.get(DataVisUtil.FISHING).toArray(new Double[chartData.get(DataVisUtil.FISHING).size()])),
					DataVisUtil.convertBaseline(chartData.get(DataVisUtil.LUMBER).toArray(new Double[chartData.get(DataVisUtil.LUMBER).size()])),
					DataVisUtil.convertBaseline(chartData.get(DataVisUtil.FIGHTER).toArray(new Double[chartData.get(DataVisUtil.FIGHTER).size()])),
					DataVisUtil.convertBaseline(chartData.get(DataVisUtil.POTION).toArray(new Double[chartData.get(DataVisUtil.POTION).size()])),
					DataVisUtil.convertBaseline(chartData.get(DataVisUtil.BUILDING).toArray(new Double[chartData.get(DataVisUtil.BUILDING).size()])),
					DataVisUtil.convertBaseline(chartData.get(DataVisUtil.MACHINE).toArray(new Double[chartData.get(DataVisUtil.MACHINE).size()])),
					DataVisUtil.convertBaseline(chartData.get(DataVisUtil.TRANSPORT).toArray(new Double[chartData.get(DataVisUtil.TRANSPORT).size()])));
	...
	private void drawLineGraph(int x, int y, int width, int height, int numElements, int numGuidelines, Double[]... values) 
	{
		...
		for (Double[] valList : values) {
			for(int j=DataVisUtil.swStart(valList.length, numElements); 
					j<DataVisUtil.swEnd(valList.length, numElements); j++) {
		...
		for (int i=0; i<values.length; i++) {
			...
			int startInd = DataVisUtil.swStart(values[i].length, numElements);
			for(int j=startInd; 
					j<DataVisUtil.swEnd(values[i].length, numElements); j++) {
